以下是针对校园问答数据适配后的清洗脚本，结合校园场景特点进行了针对性优化，主要调整包括自定义词典补充、校园专属停用词、问答噪声处理等：
...
1. **自定义词典增强**：
   - 新增校园核心实体（如“学分”、“GPA”、“辅导员”等）
   - 补充校园场景短语（如“期末考核”、“选课系统”、“交换项目”等）
   - 避免专业术语被错误分词（如“学分绩点”不会被拆分为“学分”“绩点”）

2. **噪声处理优化**：
   - 新增去除问答标记（如“Q:”、“【问题】”）
   - 过滤校园场景冗余信息（学号、校园邮箱、校内网址）
   - 清理列表序号残留（如“1.”“一、”等格式符号）

3. **停用词表扩充**：
   - 加入校园问答常用冗余词（如“同学”、“老师”、“请问”、“谢谢”）
   - 过滤无意义疑问词（如“什么”、“怎么”、“为什么”在单独出现时）

4. **基础清洗强化**：
   - 增加短文本过滤（小于5字的视为无效问答）
   - 优化去重逻辑，适应校园场景中重复提问的情况

5. **测试模块适配**：
   - 生成校园问答典型测试数据（包含选课、GPA计算、综测等场景）
   - 验证清洗效果是否符合校园问答数据特点

使用时，只需将校园问答数据（CSV格式，包含文本列）作为输入，脚本会自动完成适配场景的清洗处理。